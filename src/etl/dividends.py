...
def get_dividends(ticker: str) -> pd.DataFrame:
    ...
    params = {"ticker": ticker, "limit": 1000, "order": "asc", "sort": "ex_dividend_date", "apiKey": POLYGON_API_KEY}
    ...
    df["event_date"] = df["pay_date"].fillna(df["ex_dividend_date"]).fillna(df["record_date"]).fillna(df["declaration_date"])
    df = df[~df["event_date"].isna()]
    return df[["ticker","event_date","declaration_date","ex_dividend_date","pay_date","record_date","cash_amount"]]
...
def build():
    ...
    for t in tqdm(tickers, desc="processing dividends"):
        if parquet_exists(DIVIDENDS_DIR, t):
            skipped += 1
            continue
        success = False
        for _ in range(2):
    ...
    print(f"\nSummary Dividends: {created} created, {skipped} skipped, {failed} failed of {total} total tickers.")
